...
export default function SubtractionTablesPage() {
  ...
  const finishSession = useCallback(async (finalScore: number) => {
    ...
    if (finalScore === TOTAL) {
      ...
      speakCelebration(`Brilliant work, Wes! You got ${finalScore} out of ${TOTAL}. That's MY Wes — keep going!`);
    }
    await saveGameSession({
      skill_area: 'math_explorer',
      sub_game: 'subtraction_tables',
      score: finalScore,
  ...
  const xp = useMemo(() => calculateXp('subtraction_tables', score, TOTAL), [score]);
